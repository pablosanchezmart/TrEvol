...
#'
#' @examples
correlationsTraits <- function (VARIABLES, PHYLOGENY, DATASET, MODEL.SPECIFICATIONS = NULL,
                                FORCERUN = F)
{

  # results object
  correlationsResults <- list()
  correlationsResults$correlation.results <- data.frame()
  correlationsResults$models.diagnostics <- data.frame()
  correlationsResults$individual.models.results <- list()

  # prepare models structure
  multi_mdls.str <- expand.grid(VARIABLES, VARIABLES)
  multi_mdls.str$type <- paste0("bi_", multi_mdls.str$Var1,
                                "_", multi_mdls.str$Var2)
  multi_mdls.str$n_respVars <- 2
  multi_mdls.str$resp_var <- NA
  for (i in 1:length(multi_mdls.str$Var1)) {
    variables <- c(as.character(multi_mdls.str[i, "Var1"]),
                   as.character(multi_mdls.str[i, "Var2"]))
  ...
  multi_mdls.str$fix.frml <- paste0("cbind(", multi_mdls.str$resp_var,
                                    ") ~ trait-1")
  ...
  for (model in multi_mdls.str$type) {
    ...
    if (!model %in% names(correlationsResults$individual.models.results) | FORCERUN) {
      ...
      correlationsResults$correlation.results <- rbind(correlationsResults$correlation.results,
                                                       mdl.rslts$corrrelationsSummary)
      correlationsResults$models.diagnostics <- rbind(correlationsResults$models.diagnostics,
  ...
  print(correlationsResults$correlation.results)

  # save results
  save(correlationsResults, file = paste0(outputs.dir, "/models_outputs/correlationsResults.RData"))
  print(paste0(outputs.dir, "/models_outputs/correlationsResults.RData"))
